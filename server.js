...
app.get('/diag/decrypt-test', async (req, res) => {
    ...
    try {
        const db = require('./services/databaseService');
        const targetIds = [
            '6caf47c5-d299-4419-898e-90f7456b8402',  // Henrique
            '489c2fb8-4982-4a21-8f0f-a91b0d06c6ce',  // Raquel dependente
            'b974c27a-1ea2-4c90-ab4f-e7cf5bb7c7f0',  // Raquel (outro tel, ref OK)
        ];
        ...
        for (const id of targetIds) {
            ...
            const cpf = data.cpf;
            if (!cpf) {
                results.push({ id, name: data.name, phone: data.phone, created_at: data.created_at, hasCpf: false });
                continue;
            }

            const parts = cpf.split(':');
            if (parts.length !== 3) {
                results.push({ id, name: data.name, phone: data.phone, created_at: data.created_at, format: 'plain_or_invalid', partsCount: parts.length });
                continue;
            }

            const decrypted = db.decryptData(cpf, 'cpf');
            results.push({
                id,
                name: data.name,
                phone: data.phone,
                created_at: data.created_at,
                cipherPrefix: cpf.substring(0, 20),
                decryptOk: decrypted !== null,
                // Retorna apenas os 3 primeiros dígitos mascarados, NUNCA o CPF completo
                cpfHint: decrypted ? decrypted.substring(0, 3) + '.***.***' : null
            });
        }

        // Adiciona informação sobre a chave em uso (apenas primeiros 8 chars)
        const keyPrefix = process.env.CPF_ENCRYPTION_KEY ? process.env.CPF_ENCRYPTION_KEY.substring(0, 8) : 'UNDEFINED';
