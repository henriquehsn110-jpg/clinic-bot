...
#### 14. Ampliação do TTL de Sessão Conversacional (30 min → 24 horas / 1440 min)
- **Sintoma:** Pacientes que iniciavam o agendamento no WhatsApp de manhã (ex: 08:03 AM) e ficavam algumas horas sem responder, ao responderem mais tarde ou clicarem em "Confirmar", viam a conversa reiniciar do zero com mensagem de boas-vindas ou recebiam erro de rascunho ausente (`SCHEDULING_CONFIRMATION_FAILED`).
- **Causa Raiz:** O campo `SESSION_TTL_MINUTES` em `databaseService.js` era fixado em 30 minutos (`const SESSION_TTL_MINUTES = 30`). Quando o paciente respondia após 30 minutos, `sessions.get` identificava o TTL expirado e deletava a sessão e o rascunho do Supabase, forçando a mensagem de boas-vindas ou falha no rascunho.
- **Resolução:** Alterado `SESSION_TTL_MINUTES` para `parseInt(process.env.SESSION_TTL_MINUTES) || 1440` (24 horas). Agora o histórico e rascunho persistem com segurança durante o dia inteiro, mesmo se a aplicação na nuvem (Render) reiniciar ou receber novos deploys.

#### 15. Trava Absoluta Anti-Alucinação de Componentes Visuais (Exclusividade Mútua Calendário vs CPF/Nome)
- **Sintoma:** Em casos raros onde o rascunho da sessão era resetado ou onde o prompt continha mensagens como "Informe o seu CPF", o modelo de linguagem Gemini ou a máquina de estados avaliava `!draft.date` como verdadeiro e marcava `showCalendar: true` simultaneamente a `requireCpf: true` ou solicitação de Nome. O cliente visual do WhatsApp abria o calendário e pedia o CPF ao mesmo tempo.
- **Causa Raiz:** A máquina de estados determinística no `conversationController.js` não impunha a trava de exclusividade mútua estrita entre a solicitação de CPF/Nome e a exibição de componentes visuais (`showCalendar`, `showTimeSlots`, `showProceduresList`).
- **Resolução:** Impossibilitada a sobreposição via **Trava Absoluta de Exclusividade Mútua** no `conversationController.js`: quando `requireCpf` ou `wasCpfRequested` ou `wasNameRequested` ou a mensagem contiver busca de CPF/Nome, `showCalendar`, `showTimeSlots` e `showProceduresList` são forçados deterministicamente a `false`. Adicionada verificação de regressão em `.agents/scripts/check_regressions.js` e teste unitário em `tests/test_component_exclusivity.js`.
