...
30. **Adesão Estrita ao Escopo e Remoção de Código Órfão (Anti-Fuga de Escopo):** NENHUM arquivo fora do escopo explicitamente instruído pode ser incluído em um commit, sob nenhuma circunstância. Se uma rotina de CI quebrar por dependência de um código não solicitado (ex: um `require` de um arquivo inexistente ou untracked inserido por um agente), a única correção aceitável é REMOVER a referência intrusa do código-fonte. É estritamente proibido criar arquivos novos, inventar dependências ou commitar scripts não solicitados apenas para "fazer a CI passar". Além disso, toda validação final antes de solicitar aprovação de merge deve obrigatoriamente apresentar o output bruto de `git diff --stat` contra a branch base, provando inequivocamente que a superfície tocada é apenas a exigida.
31. **Disclosure Completo de Diffs (Anti-Recorte):** Toda vez que um diff for apresentado para aprovação humana, deve ser o `git show --stat <commit>` completo do commit inteiro, seguido do diff completo de CADA arquivo tocado — nunca um recorte de linhas selecionadas pelo próprio agente. Um recorte parcial, mesmo que tecnicamente verdadeiro sobre o trecho mostrado, é uma forma de disclosure incompleto e trata-se como violação da Regra 11 caso venha a ocultar mudança relevante.
